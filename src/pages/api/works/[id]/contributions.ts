...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  try {
    const work = await creditWork(id, { name: payload.name, role: payload.role }, env);
    ...
    console.error('Error crediting Work:', error);
    return jsonResponse({ error: 'Failed to credit Work' }, 500);
  }
};
